...
class EntityFactory(private val assetManager: AssetManager, private val localAssetManager: AssetManager) {
    ...
    private val soundFactory = SoundFactory(assetManager)
    ...
    fun createBubble(listener: InputListener, onAutomaticBlowUp: (bubble: BubbleEntity) -> Unit) = BubbleEntity(soundFactory.getBlowUp(),assetManager.get(Constant.Texture.ExplodingBubble.tName), assetManager.get(Constant.Texture.BubbleGame.tName), listener, onAutomaticBlowUp)
    fun createBomb(listener: InputListener, onAutomaticBlowUp: (bomb: BombEntity) -> Unit) = BombEntity(soundFactory.getBombFuse(), soundFactory.getBombExplotion(), assetManager.get(Constant.Texture.Bomb.tName), assetManager.get(Constant.Texture.ExplodingBomb.tName), assetManager.get(Constant.Texture.BombExplotion.tName), listener, onAutomaticBlowUp)
