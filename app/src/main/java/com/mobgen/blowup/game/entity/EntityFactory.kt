...
class EntityFactory(private val assetManager: AssetManager, private val localAssetManager: AssetManager) {
    ...
    fun createBubble(listener: InputListener, onAutomaticBlowUp: (bubble: BubbleEntity) -> Unit) = BubbleEntity(soundFactory.getBlowUp(), assetManager.get(Constant.Texture.BubbleGame.tName), listener, onAutomaticBlowUp)
    fun createBomb(listener: InputListener, onAutomaticBlowUp: (bubble: BombEntity) -> Unit) = BombEntity(soundFactory.getBombFuse(), soundFactory.getBombExplotion(), assetManager.get(Constant.Texture.Bomb.tName), listener, onAutomaticBlowUp)
